                    ...
                    <div class="table-responsive">
                        <table class="table  table-bordered">
                            ...
                            <c:forEach items="${page.list}" var="admin" varStatus="status">
                                <tr>
                                    <td>${status.count}</td>
                                    <td><input type="checkbox" class="itemCheckboxClass" adminId="${admin.id}"></td>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-success btn-xs"><i
                                                ...
                                                onclick="assignRole('+${admin.id}+')"></i></button>
                                        <button type="button" class="btn btn-primary btn-xs"
                                                onclick="window.location.href='${PATH}/admin/toUpdate?id=${admin.id}&pageNum=${page.pageNum}'">
                                            <i class=" glyphicon glyphicon-pencil"></i></button>
                                        <button type="button" id="deleteAdmin"
                                                onclick="deleteAdmin('${admin.loginacct}',${admin.id})"
                                    ...
                                    <ul class="pagination">
                                        ...
                                        <c:forEach items="${page.navigatepageNums}" var="num">
                                            <c:if test="${num == page.pageNum}">
                                                <li class="active"><a
                                                        href="${PATH}/admin/index?pageNum=${num}&condition=${param.condition}">${num}</a>
                                                </li>
                                            </c:if>
                                            <c:if test="${num != page.pageNum}">
                                                <li>
                                                    <a href="${PATH}/admin/index?pageNum=${num}&condition=${param.condition}">${num}</a>
                                                </li>
                                            </c:if>
                                        </c:forEach>
...
<script type="text/javascript">
    ...
    function deleteAdmin(loginacct, id) {
        //弹层组件
        layer.confirm("你确定要删除【" + loginacct + "】吗？", {btn: ['确定', '取消']}, function () {
            window.location.href = "${PATH}/admin/deleteAdmin?id=" + id + "&pageNum=${page.pageNum}";
        }, function () {
        });
    }
    ...
    $("#threadCheckbox").click(function () {
        let threadChecked = $(this).prop("checked");
        $(".itemCheckboxClass").prop("checked", threadChecked);
    });
    ...
    $("#deleteBatchBtn").click(function () {
        ...
        if(checkedBoxList.length===0){
            layer.msg("请先选择要删除的账号！");
            return false;
        }
        let idArray = [];
        $.each(checkedBoxList, function (i, e) {//i索引  元素
            let adminId = $(e).attr("adminId");//将dom对象转换成
    ...
    function assignRole(adminId) {
        window.location.href="${PATH}/admin/assignRole?adminId="+adminId;
    }
</script>
</body>
</html>
